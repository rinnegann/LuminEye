...
def dice_loss_function(y0,y): # Final Argument== Mask
    
    return  dice_loss(y0,y)
...
def fit(epochs, model, train_loader, val_loader, optimizer, scheduler, patch=False):
    ...
    for e in range(epochs):
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            #Multi DiceBceLoss
            loss = dice_loss_function(y0,mask)
            ...
            with torch.no_grad():
                for i, data in enumerate(tqdm(val_loader)):
                    ...
                    loss = dice_loss_function(y0_l,mask)
