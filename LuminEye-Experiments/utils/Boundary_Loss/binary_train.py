...
from losses import GeneralizedDice
from losses import SurfaceLoss
from local_losses import IoU,get_lr,pixel_wise_accuracy,DiceLoss
...
def get_images(train_x,train_y,test_x,test_y,batch_size=1,shuffle=True,pin_memory=True):
    train_data = Iris(train_x,train_y,(416,416))
    val_data  = Iris(test_x,test_y,(416,416))
...
train_batch,val_batch = get_images(train_x,train_y,valid_x,valid_y,batch_size=batch_size)
...
def fit(epochs, model, train_loader, val_loader, optimizer, scheduler, patch=False):
    ...
    boundary_loss = SurfaceLoss(idc=[0,1])
    ...
    for e in range(epochs):
        ...
        for i, data in enumerate(tqdm(train_loader)):
            #training phase
            image_tiles,mask_tiles,dist_maps = data["images"],data["gt"],data["dist_map"]
            # image_tiles= image_tiles.to(device)
            # mask_tiles = mask_tiles.to(device)
            
            # image = image_tiles.to(device)
            image = image_tiles.type(torch.cuda.FloatTensor)
            mask = mask_tiles.to(device)
            dist_map_label = dist_maps.to(device)
            ...
            # print(f"GT: {mask.shape}")
            
            pred_probs = F.softmax(output, dim=1)
            
            # print(f"Predicted Probs: {pred_probs.shape}")
            # print(f"Distance Maps: {dist_map_label.shape}")
            gdl_loss = dice_loss(output,mask)
            
            bl_loss = boundary_loss(pred_probs, dist_map_label)  # Notice we do not give the same input to that loss
            loss = gdl_loss + α * bl_loss
